...
const verifyOtp = async (req, res) => {
  try {
    ...
    const findOtp = await OTP.find({ userid: userID })
      .sort({ createdAt: -1 })
      .limit(1);
    console.log(findOtp);
    ...
    if (findOtp.length > 0) {
      ...
      if (verifyOtp) {
        await user
          .updateOne({ _id: userID }, { $set: { is_verified: true } })
          .catch((err) => {
            console.log(err);
          });
          ...
          if (walletData) {
            await wallet.updateOne(
              { userId: req.session.refferalId },
              {
                $inc: { balance: 100 },
                $push: {
                  transaction: { amount: 100, transactionsMethod: "refferal" },
                },
              }
            );
          } else {
...
const Homepage = async (req, res) => {
  ...
  try {
    ...
    let productData = await products
      .find({ is_blocked: false, is_categoryBlocked: false })
      .sort({ _id: -1 })
      .limit(limit)
      .skip(skip)
      .populate("categoryId");
    ...
    productData = productData.map((product) => {
      ...
      offerData.forEach((offer) => {
        if (
          ...
          offer.categoryId.includes(product.categoryId._id.toString())
        ) {
